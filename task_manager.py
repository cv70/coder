...
class TaskManager:
    ...
    def _next_id(self) -> int:
        ids = [
            int(f.stem.split("_")[1]) for f in (TASK_DIR).glob("task_*.json")
        ]
        return max(ids, default=0) + 1

    def _load(self, tid: int) -> dict:
        p = TASK_DIR / f"task_{tid}.json"
        if not p.exists():
            raise ValueError(f"Task {tid} not found")
        return json.loads(p.read_text())

    def _save(self, task: dict):
        (TASK_DIR / f"task_{task['id']}.json").write_text(
            ...
            if status == "completed":
                for f in (TASK_DIR).glob("task_*.json"):
                    ...
                    if tid in t.get("blockedBy", []):
                        ...
                        self._save(t)
            if status == "deleted":
                (TASK_DIR / f"task_{tid}.json").unlink(missing_ok=True)
